...
test_data = load_images_from_folder(test_path).iloc[:, :]
...
ccp_alpha_grid_search = GridSearchCV(estimator=DecisionTreeClassifier(random_state=38), param_grid={'ccp_alpha': ccp_alphas}, cv=5, n_jobs=-1)
ccp_alpha_grid_search.fit(train_data.iloc[:,:-1], train_data.iloc[:,-1])
end_time = time.time()
best_para = ccp_alpha_grid_search.best_params_
dt2 = ccp_alpha_grid_search.best_estimator_

# Evaluate the best tree on the training and validation set

train_acc = dt2.score(train_data.iloc[:,:-1], train_data.iloc[:,-1])
val_acc = dt2.score(val_data.iloc[:,:-1], val_data.iloc[:,-1])
...
plot_tree(dt2, filled=True, rounded=True, class_names=['car', 'person', 'airplane', 'dog'])
plt.show()

# Print the confusion matrix
y_pred = dt2.predict(val_data.iloc[:,:-1])
